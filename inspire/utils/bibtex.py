...
class Bibtex(object):
    ...
    def format(self):
        ...
        entry_type, original_entry = self._get_entry_type()
        if entry_type in formats:
            return formats[entry_type]()
    ...
    def _is_arxiv_present(self):
        """Return if archive is present"""
    ...
    def _get_booktitle(self):
        ...
        entry_type, original_entry = self._get_entry_type()
        if entry_type == 'inproceedings' or original_entry == 'inproceedings':
            from inspire.utils import bibtex_booktitle
    ...
    def _get_year(self):
        ...
        if 'publication_info' in self.record:
            ...
            if isinstance(pub_info, list):
                ...
                if not year:
                    if 'imprint' in self.record:
                        imprint = self.record['imprint']
                        for date in imprint:
                            if 'date' in date:
                                year = date['date'].split('-')[0]
                    elif 'preprint_info' in self.record:
                        preprint_info = self.record['preprint_info']
                        for date in preprint_info:
                            if 'date' in date:
                                year = date['date'].split('-')[0]
            else:
                if 'year' in pub_info:
        ...
        elif original_entry == 'thesis' and 'thesis' in self.record:
                ...
                for index, val in enumerate(pub_info):
                    if index == 0:
                        if 'journal_title' in val:
                            if isinstance(val['journal_title'], list):
                                journal_title = val['journal_title'][-1]
                            else:
                ...
                for index, val in enumerate(pub_info):
                    if index == 0:
                        if 'journal_volume' in val:
                            if 'journal_title' in val and \
                                            val['journal_title'] == 'JHEP':
                                volume = re.sub(r'\d\d(\d\d)', r'\1',
                                                val['journal_volume'])
                            else:
                                volume = val['journal_volume']
            else:
                ...
                for index, val in enumerate(pub_info):
                    if index == 0:
                        if 'page_artid' in val:
                            if isinstance(val['page_artid'], list):
                                pages = val['page_artid'][-1]
                            else:
                                pages = val['page_artid']
            else:
                        ...
                        if index >= 1:
                            ...
                            if 'note' not in val and \
                                ...
                                if 'year' in val:
                                    ...
                                    year = self.record['preprint_info']
                                    for date in year:
                                        if 'date' in date:
                                            note += "(" + date['date']\
                                                    .split('-')[0] + ")"
                                ...
                                (val['note'] == 'Erratum' or
                                 val['note'] == 'Addendum' or
                                 val['note'] == 'Corrigendum' or
                                 val['note'] == 'Reprint'):
                                if 'journal_title' in val:
                                    note += re.sub(r'\.([A-Z])', r'. \1',
                                                   val['journal_title'])
            ...
            for field in self.record['report_number']:
                if ('source' in field and field['source'] == 'arXiv') \
                    ...
                    if field['primary'].upper().startswith('ARXIV:'):
                        result.append(field['primary'][6:])
                    else:
                        result.append(field['primary'])
        if result:
        ...
        if self._is_arxiv_present():
            for field in self.record['report_number']:
                if ('source' in field and field['source'] == 'arXiv') \
                    or 'arxiv_category' in field or \
                    ('primary' in field
                        and field['primary'].upper().startswith('ARXIV:')):
                    if 'primary' in field:
