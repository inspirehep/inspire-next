...
class Citation(db.Model):
    ...
    citee = db.Column(db.Integer, primary_key=True)
    citer = db.Column(db.Integer, primary_key=True)
...
class Citation_Log(db.Model):
    ...
    citation_type =  db.Column(db.Enum('added','removed',name='citation_type'), default='added'  )
    ...
    def save(self):
        if self.citation_type == 'removed':
            cit = Citation.query.filter_by(citee=self.citee,citer=self.citer).first()
            if cit != None:
                print("deleting")
