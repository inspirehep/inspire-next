...
def load_submitted_record(input_data):
    def _load_submitted_record():
        return (
            'M. Twain' in record and 'Twain, Mark' in record and
            'Experiment: ATLAS - From: 2002 To: 2005' in record and
            'Submitted by admin@inspirehep.net\non' in record and
            'Some comments about the author' in record and 'http://www.example1.com' in record and
            'http://www.example3.com' in record and 'http://www.example4.com' in record and
            'http://www.example5.com' in record and 'ACC-PHYS' in record and
            'ASTRO-PH' in record and 'Bob White' in record and 'CERN' in record and
            '2000' in record and '2001' in record
        )
    ...
    try:
        ...
        record += WebDriverWait(
            Arsenic(), 10
        ).until(EC.visibility_of_element_located((By.XPATH, '(//div[@class="detail-panel"])[4]'))
                ).text
        record += WebDriverWait(
            Arsenic(), 10
        ).until(EC.visibility_of_element_located((By.XPATH, '(//div[@class="detail-panel"])[5]'))
                ).text
        record += WebDriverWait(Arsenic(), 10).until(
            EC.visibility_of_element_located((
                By.XPATH, '(//div[@class="col-md-9 col-sm-9 col-xs-8 ng-binding"])[2]'
            ))
        ).text
        record += WebDriverWait(Arsenic(), 10).until(
            EC.visibility_of_element_located((By.XPATH, '//div[@class="detail-panel panel-100"]'))
        ).text
        record += WebDriverWait(
            Arsenic(), 10
        ).until(EC.visibility_of_element_located((By.XPATH, '//div[@class="subject-list"]'))).text
        record += WebDriverWait(
            Arsenic(), 10
        ).until(EC.visibility_of_element_located((By.XPATH, '//div[@class="advisors"]'))).text
        record += WebDriverWait(
            Arsenic(), 10
        ).until(EC.visibility_of_element_located((By.XPATH, '//div[@class="table-body"]'))).text
...
def reject_record():
    def _reject_record():
        return 'Rejected' in WebDriverWait(Arsenic(), 10).until(
            EC.visibility_of_element_located((
                By.XPATH, '//div[@class="alert ng-scope alert-reject"]'
            ))
        ).text
...
def curation_record():
    def _curation_record():
        return 'Accepted with Curation' in WebDriverWait(Arsenic(), 10).until(
            EC.visibility_of_element_located((By.XPATH, '//span[@ng-switch-when="accept_curate"]'))
        ).text
    ...
    def _review_record():
        return (
            WebDriverWait(Arsenic(), 10
                          ).until(EC.visibility_of_element_located((By.ID, 'inspireid'))) and
