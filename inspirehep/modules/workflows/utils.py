...
def json_api_request(url, data, headers=None):
    ...
    try:
        response = requests.post(url=url, headers=final_headers, data=json.dumps(data), timeout=30)
...
def log_workflows_action(action, prediction_results, object_id, user_id, source, user_action=""):
