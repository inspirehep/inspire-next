      ...
      {% if record.admin_tools %}
        <div class="col-md-12" id="admin-tools">
          {% for tool in record.admin_tools %}
            {% if tool == 'editor' %}
              <a href="/editor/record/jobs/{{record.control_number}}"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>
            {% endif %}
          {% endfor %}
        </div>
      {% endif %}
      ...
      <div class="col-md-4 hidden-xs hidden-sm" id="record-detailed-jobs-deadline">
        {% if record.institutions[0].name %}
          <div class="detailed-map" id="job-detailed-map">
              ...
              function geocodeAddress(geocoder, resultsMap, address) {
                geocoder.geocode({'address': address}, function(results, status) {
                  if (status === google.maps.GeocoderStatus.OK) {
                    ...
                    var image = {
                      url: '/assets/images/map/marker-institutions.png',
            ...
            <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyA0JQa0S94TdFfXHsj7JKXjVN9mu0FighU&callback=initMap" async defer></script>
            </div>
        {% endif %}
      </div>
