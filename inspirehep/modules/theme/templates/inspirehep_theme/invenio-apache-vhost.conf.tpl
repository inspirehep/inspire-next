...
#}
{%- extends "inspirehep_theme/invenio-apache-vhost.tpl" -%}
...
{%- block logging %}
        ErrorLog {{ config.CFG_APACHE_LOGDIR }}/apache{{ log_suffix }}.err
        LogLevel warn
        LogFormat "%h %l %u %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-agent}i\" %D" combined_with_timing
        CustomLog {{ config.CFG_APACHE_LOGDIR }}/apache{{ log_suffix }}.log combined_with_timing
{%- endblock logging -%}
