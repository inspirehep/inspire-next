  ...
  function booksTypeahead($element) {
    $element.extendedTypeahead({
      ...
      dataEngine: new Bloodhound({
        name: 'books',
        remote: {
          url: '/api/literature?q=bookautocomplete:%QUERY*',
          filter: function(response) {
            return $.map(response.hits.hits, function(el) {
              console.log(el.metadata)
              el.metadata.title = el.metadata.titles[0].title;
              if(typeof el.metadata.isbns !== 'undefined'){
                el.metadata.isbn = el.metadata.isbns[0].value;
              }
              if(typeof el.metadata.earliest_date !== 'undefined'){
                el.metadata.date = el.metadata.earliest_date;
              }
              return el.metadata
            })
          }
