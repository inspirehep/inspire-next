...
class ConferenceNode(RecordNode):
    ...
    default_labels = RecordNode.default_labels | set(
        [NodeLabels.Conference])


class ConferenceSeriesNode(BaseNodeType):

    default_labels = set([NodeLabels.ConferenceSeries])
    create_if_does_not_exist = True

    @staticmethod
    def generate_uid(name):
        return UID_BASIC_SEPARATOR.join(['ConferenceSeries', name])

    @classmethod
    def create_graph_dict_from_uid(cls, uid):

        _, name = uid.split(UID_BASIC_SEPARATOR)

        model = create_graph_dict(cls)
        set_uid(model, uid)
        set_property_of_central_node(model, 'name', name)
