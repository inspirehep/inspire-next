...
class LiteratureCitationsResource(ContentNegotiatedMethodView):
    ...
    def get(self, pid, record):
        ...
        citing_records_results = record.get_citing_records_query.paginate(page, size, False)
        citing_records_uuids = map(lambda results: results[0], citing_records_results.items)
        citing_records = InspireRecord.get_records(citing_records_uuids)

        return self.make_response(pid, citing_records)
