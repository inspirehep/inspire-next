...
@pytest.mark.xfail
def test_private_notes_from_595_Da():
    schema = load_schema('hep')
...
def test_private_notes_from_595_Ha():
    ...
    snippet = (
        ...
        '  <subfield code="a">En attente de modification d\'une affiliation - Dominique</subfield>'
        '</datafield>'
    )  # record/1513573
    ...
    expected = [
        {
            'source': 'HAL',
            'value': 'En attente de modification d\'une affiliation - Dominique',
        },
    ]
    result = hep.do(create_record(snippet))
    ...
    expected = {
        'a': [
            'En attente de modification d\'une affiliation - Dominique',
        ],
    }
    result = hep2marc.do(result)
