...
def test_harvesting_arxiv_workflow_accepted(
    ...
    with small_app.app_context():
        ...
        # Check the conversion for the legacy sys
        marc_json = hep2marc.do(obj.data)
        marcxml = legacy_export_as_marc(marc_json)
        record = create_record(marcxml)

        assert '1' == record.get('001')

        assert '10.1063/1.4890289' == record.get('024__').get('a')

        assert 'oai:arXiv.org:1407.7587' == record.get('035__').get('a')

        assert 'CUQM---149' == record.get('037__')[0].get('a')

        assert 'arXiv:1407.7587' == record.get('037__')[1].get('a')

        assert 'math-ph' == record.get('037__')[1].get('c')

        assert 'Hall, Richard L' == record.get('100__').get('a')

        assert 'Spectra generated by a confined softcore Coulomb potential' == record.get('245__').get('a')

        assert '2014-07-28' == record.get('269__').get('c')

        assert '17' == record.get('300__').get('a')

        assert '17 pages, 2 figures' == record.get('500__')[0].get('a')

        assert '*Temporary entry*' == record.get('500__')[1].get('a')

        assert 'Analytic and approximate solutions for the energy eigenvalues ' \
            'generated by a confined softcore Coulomb potentials of the form a/(r+\\beta) ' \
            'in d>1 dimensions are constructed. The confinement is effected by linear and ' \
            'harmonic-oscillator potential terms, and also through `hard confinement\' ' \
            'by means of an impenetrable spherical box. A byproduct of this work is the ' \
            'construction of polynomial solutions for a number of linear differential ' \
            'equations with polynomial coefficients, along with the necessary and ' \
            'sufficient conditions for the existence of such solutions. Very accurate ' \
            'approximate solutions for the general problem with arbitrary potential ' \
            'parameters are found by use of the asymptotic iteration method.' == record.get('520__').get('a')

        assert 'http://arxiv.org/licenses/nonexclusive-distrib/1.0/' == record.get('540__').get('u')

        assert 'OAI' == record.get('541__').get('c')

        assert 'math-ph' == record.get('65017')[0].get('a')

        assert 'math.MP' == record.get('65017')[1].get('a')

        assert 'hep-ex' == record.get('65017')[2].get('a')

        assert 'potential: Coulomb' == record.get('695__')[0].get('a')

        assert 'numerical calculations' == record.get('695__')[1].get('a')
        assert 'potential: confinement' == record.get('695__')[2].get('a')
        assert 'differential equations' == record.get('695__')[3].get('a')
        assert 'potential: linear' == record.get('695__')[4].get('a')
        assert 'confinement' == record.get('695__')[5].get('a')
        assert 'asymptotic behavior' == record.get('695__')[6].get('a')
        assert 'potential' == record.get('695__')[7].get('a')
        assert 'analytic properties' == record.get('695__')[8].get('a')

        assert 'Saad, Nasser' == record.get('700__').get('a')

        assert '082102' == record.get('773__').get('c')
        assert 'J. Math. Phys.' == record.get('773__').get('p')
        assert '55' == record.get('773__').get('v')
        assert '2014' == record.get('773__').get('y')
        assert 'J. Math. Phys. 55, 082102 (2014)' == record.get('773__').get('x')

        assert 'CORE' == record.get('980__')[0].get('a')
        assert 'arXiv' == record.get('980__')[1].get('a')
        assert 'Citeable' == record.get('980__')[2].get('a')
        assert 'HEP' == record.get('980__')[3].get('a')
