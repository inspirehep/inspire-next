...
from inspirehep.modules.workflows.tasks.upload import (is_stale_data,
                                                       store_record,
                                                       store_root)
...
def test_store_root_update_record(workflow_app):
    ...
    with patch.dict(current_app.config, config):
        ...
        insert_wf_record_source(
            json_data=original_root, record_uuid=head_uuid, source="arxiv"
        )
...
def test_regression_store_record_does_not_commit_when_error(workflow_app):
    ...
    with patch.object(
        InspireRecord, "download_documents_and_figures", side_effect=Exception
    ):
    ...
    with patch.dict(
        ...
        with requests_mock.Mocker() as requests_mocker:
            requests_mocker.register_uri(
                ...
                status_code=201,
                json={
                    "metadata": {"control_number": expected_control_number},
...
def test_store_record_inspirehep_api_literature_new_has_the_if_match_headers(
    mock_put_record_to_hep, workflow_app
):
    expected_control_number = 111
    expected_head_uuid = str(uuid.uuid4())
    expected_version = '"1"'
    ...
    assert (
        expected_version
        == mock_put_record_to_hep.call_args_list[0][1]["headers"]["If-Match"]
    )
    assert workflow.data["control_number"] == expected_control_number
    assert workflow.extra_data["recid"] == expected_control_number
    assert workflow.extra_data["head_uuid"] == expected_head_uuid
...
def test_store_record_inspirehep_api_literature_update_not_marked_as_update(
    ...
    record_data = {
        ...
        "extra_data": {
            "source_data": {
                ...
                "extra_data": {},
                "persistent_data": {"marks": {"restart-count": 0}},
            },
        },
    }

    workflow = workflow_object_class.create(record_data)
    obj = build_workflow(data_type="hep", **record_data)
    wf_id = obj.id
    workflow.extra_data["is-update"] = False
    ...
    with patch.dict(
        ...
        with requests_mock.Mocker() as requests_mocker:
                ...
                [
                    {
                        ...
                        "json": {
                            "status": 400,
                            "message": "PIDAlreadyExists: pid_type:'arxiv', pid_value:'2211.15599'.",
                        },
                        "headers": {"content-type": "application/json"},
                    }
                ],
            ...
            store_record(obj, eng)  # not throwing exception
            wf = workflow_object_class.get(wf_id)
            assert (
                wf.extra_data["source_data"]["persistent_data"]["marks"][
                    "restart-count"
                ]
                == 1
            )
...
def test_store_record_inspirehep_api_author_new(workflow_app):
    ...
    with patch.dict(
        ...
        with requests_mock.Mocker() as requests_mocker:
            requests_mocker.register_uri(
                "POST",
                "{url}/authors".format(url=workflow_app.config.get("INSPIREHEP_URL")),
                headers={"content-type": "application/json"},
                status_code=201,
                json={
                    "metadata": {"control_number": expected_control_number},
        ...
        with requests_mock.Mocker() as requests_mocker:
            requests_mocker.register_uri(
                ...
                headers={
                    ...
                    "If-Match": '"{}"'.format(expected_head_version_id),
                },
                status_code=200,
                json={
                    "metadata": {"control_number": expected_control_number},
...
def test_store_record_inspirehep_api_retries_on_bad_gateway(
    ...
    assert obj.extra_data["recid"] == expected_control_number
    assert obj.extra_data["head_uuid"] == expected_head_uuid
    assert obj.extra_data["url"] == "http://localhost:5000/record/222"
...
def test_store_record_inspirehep_api_literature_new_wrong_response_code(
    mocked_api_request_magpie, mocked_api_request_classifier, workflow_app
):
    record_data = {
        "workflow_data": {
