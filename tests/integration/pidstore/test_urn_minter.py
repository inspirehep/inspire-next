...
from inspirehep.modules.pidstore.minters import UrnMinter
...
def test_urn_minter_happy(isolated_app, record):
    ...
    assert len(pids) == 2
    for pid in pids:
        assert pid.pid_type == 'urn'
        assert pid.object_type == 'rec'
        assert pid.object_uuid == record.id
        assert pid.status == PIDStatus.RESERVED
    assert sorted([pid.pid_value for pid in pids]) == ['urn1', 'urn2']


def test_urn_minter_duplicated(isolated_app, record):
    # Mint a PID.
    minter = UrnMinter()
    minter.mint(str(record.id), record)
...
def test_urn_minter_with_existent_pids(isolated_app, record_w_existent_pids):
